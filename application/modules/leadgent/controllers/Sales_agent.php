...
class Sales_agent extends MY_Controller
{
	...
	public function index()
	{
		...
		$records['agent_tasks'] = $this->Agents_Model->agent_tasks($this->session->userdata['userlogin']['user_id']);
		...
		$this->load->view('sales_agent', $records);
   ...
   public function fetch_task_data(){
	...
	public function save_agent_tasks() {
        ...
        $leadgent_task_id = $this->input->post('leadgent_task_id');
        ...
        foreach ($leads as $index => $lead_id) {
			...
			$get_previous_agent_data = $this->Agents_Model->select_single_row_agent_lead($lead_id);

            $agent_tasks_data[] = [
                'lead_id' => $lead_id,
                'leadgent_task_id' => $leadgent_task_id[$index],
                'user_id' => $assign_to,
                'leadgent_user_id' => $this->session->userdata['userlogin']['user_id'],
                // 'remarks' => $remarks,
                'Priority' => "Pending",
				'status_assign' => "Not yet open",
				'additional_book' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["additional_book"],
				'pitched_price' => $get_previous_agent_data == false ? 0 : $get_previous_agent_data["pitched_price"],
				'pitched_price_marketing' => $get_previous_agent_data == false ? 0 : $get_previous_agent_data["pitched_price_marketing"],
				'pitched_price_packages' => $get_previous_agent_data == false ? 0 : $get_previous_agent_data["pitched_price_packages"],
				'services_status' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["services_status"],
				'service_purchased' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["service_purchased"],
				'payment_status' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["payment_status"],
				'agent_remarks' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["agent_remarks"],
				'amount' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["amount"],
				'agent_priority' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["agent_priority"],
				'recording' => $get_previous_agent_data == false ? "" : $get_previous_agent_data["recording"],
				'agent_date_assigned' => date('Y-m-d H:i:s'),

            ];
		
			$this->Leads_Model->update(array('lead_status_assign' => 1), $lead_id);
			$this->Recycle_Model->update(array('status' => 'Reassign'), $lead_id);
	...
	public function fetch_agent_view_recycle_data($user_id = 0, $date_assigned = "") {
		$data = $this->Agents_Model->select_agent_recycle_task($user_id, $date_assigned);
	...
	public function view_agent_task_detail() { 
		$data=array();
		$task_data =$this->Agents_Model->select_agent_user_task($this->input->get('agent_task_id')); 
		$data = $task_data;
		echo json_encode($data);             
}

public function view_agent_recycle_task_detail() { 
	$data=array();
	$task_data =$this->Agents_Model->select_agent_user_recycle_task($this->input->get('agent_task_id')); 
	$data = $task_data;
	echo json_encode($data);             
}
	...
	public function save_recyle() {
        ...
        foreach ($combinedArray  as $key => $lead_agent) {
			// $this->Agents_Model->update_agent_tasks($id, $data);
				$data =  array(
					...
					'date_recycle' =>  date("Y-m-d H:i:s"),					
				);
				$this->Agents_Model->insert_data($data);

				$data_activities = array(
					'lead_id' =>  $lead_agent['lead_id'],
					'remarks' => "Pipe and Sold Recyled Leads",
					'admin_id' => 1,
					'unread_admin' => 1,
					'status_activity' => 6,
					'user_charge' => $this->session->userdata['userlogin']['full_name'],
		...
		$this->Leads_Model->update_lead_status_assign(array('recycle_status' => 1), explode(",", $checked_Lead));
